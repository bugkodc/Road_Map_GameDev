...
```mermaid
flowchart TD
  A["Ban đầu<br/>Feature mới: 2 ngày"] --> B["Sau 6 tháng<br/>Feature mới: 5 ngày"]
  B --> C["Sau 1 năm<br/>Feature mới: 2 tuần"]
  C --> D["Sau 2 năm<br/>Gần như bất khả thi"]
```
...
```mermaid
flowchart TD
  Debt["Nợ kỹ thuật phổ biến trong Unity"]
  Debt --> God["God class GameManager<br/>3000+ dòng code"]
  Debt --> Find["Find(PlayerObject)<br/>hardcode khắp nơi"]
  Debt --> Magic["Magic number<br/>health < 0.3f"]
  Debt --> Singleton["Lạm dụng Singleton"]
  Debt --> Update["Update() chứa logic nặng<br/>không cần chạy mỗi frame"]
```
